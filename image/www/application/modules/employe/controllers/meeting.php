...
class Meeting extends CI_Controller{
	...
	function index(){
		if($this->session->userdata("logged_in") == "Assalamualaikum" )
	    {
        	...
        	$date=date('Y-m-d');
	    	$att=$this->db->query("SELECT emp_id,att_name,att_email,att_date,att_status FROM hris_attendance WHERE att_email='".$this->session->userdata('email')."' AND att_date='".$date."' AND att_status='Clock in' AND osw_id=1");
	    	$row = $att->row();
	    	if ($row != null){
	    			...
	    			if($row_out != null){
					    ...
					    $this->load->view('meeting/form_att_meeting_out',$data);
				...
				$this->load->view('v_header_emp',$d);		    		
			    $this->load->view('meeting/form_att_meeting_in',$data);
			    $this->load->view('v_footer_emp');
	    	}
